#!/bin/bash
#SBATCH --job-name=mlp_baseline_celeba_sb
...
# Non-graph MLP baseline for CelebA-Smiling (sb) (Gap 2). Trains an MLP directly on
# the raw-embeddings file (features + label, SMSL flag column stripped)
# at data/CELEBAEmbeddings/celeba_sb_train_embeddings.npy --
# no KNN graph involved -- and appends one row to
# results/mlp_baseline/celeba_sb.csv. Only needs config/mlp/celeba_sb.yaml, so
# this does not depend on the KNN graph step and can run as soon as
# that embeddings file exists.
echo "Starting MLP baseline training (CelebA-Smiling (sb))..."
