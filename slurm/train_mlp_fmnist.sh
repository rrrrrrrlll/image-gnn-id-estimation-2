#!/bin/bash
#SBATCH --job-name=mlp_baseline_fmnist
...
# Non-graph MLP baseline for FMNIST (Gap 2). Trains an MLP directly on
# the Step 5 VAE embeddings -- no KNN graph involved -- and appends one
# row to results/mlp_baseline/fmnist.csv. Only needs config/mlp/fmnist.yaml,
...
echo "Starting MLP baseline training (FMNIST)..."
python src/scripts/train_mlp_baseline.py \
    -d config/mlp/fmnist.yaml \
