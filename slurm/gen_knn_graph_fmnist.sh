#!/bin/bash
#SBATCH --job-name=gen_knn_fmnist
...
# Build the KNN graph for FMNIST (README Step 9).
# Reads config/knn/fmnist.yaml (the full-dimension, non-"_pca_" embedding
# files after the dimensionality fix) and writes
# data/FMNISTGraph/fmnist_{train,test}_knn_graph-100.pkl,
# which config/gnn/fmnist.yaml then points training at.
#
...
# rough estimate, not measured -- FMNIST showed roughly 40+ it/s on comparable low-dimensional embeddings during a
# manual run. Adjust once you see a real completion time.
echo "Starting KNN graph generation (FMNIST)..."
python src/scripts/gen_knn_graphs.py \
    -d config/knn/fmnist.yaml \
    -k 100 \
    --dataset_name fmnist
