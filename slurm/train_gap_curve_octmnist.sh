#!/bin/bash
#SBATCH --job-name=gap_curve_octmnist
...
# Generalization-gap-vs-graph-size curve for OCTMNIST (Gap 1).
...
# one row per (size, seed) to results/gap_curve/octmnist.csv.
# Requires config/gnn/octmnist.yaml to point at the OCTMNIST KNN graph
# produced by src/scripts/gen_knn_graphs.py (README Step 9) -- same
# prerequisite as train_gnn_octmnist.sh.
echo "Starting Gap-1 curve training (OCTMNIST)..."
python src/scripts/train_gap_curve.py \
    -d config/gnn/octmnist.yaml \
