#!/bin/bash
#SBATCH --job-name=mlp_baseline_cifar10
...
# Non-graph MLP baseline for CIFAR10 (Gap 2). Trains an MLP directly on
# the Step 5 VAE embeddings -- no KNN graph involved -- and appends one
# row to results/mlp_baseline/cifar10.csv. Only needs config/mlp/cifar10.yaml,
...
echo "Starting MLP baseline training (CIFAR10)..."
python src/scripts/train_mlp_baseline.py \
    -d config/mlp/cifar10.yaml \
