#!/bin/bash
#SBATCH --job-name=mlp_baseline_pathmnist
...
# Non-graph MLP baseline for PathMNIST (Gap 2). Trains an MLP directly on
# the Step 5 VAE embeddings -- no KNN graph involved -- and appends one
# row to results/mlp_baseline/pathmnist.csv. Only needs config/mlp/pathmnist.yaml,
...
echo "Starting MLP baseline training (PathMNIST)..."
python src/scripts/train_mlp_baseline.py \
    -d config/mlp/pathmnist.yaml \
