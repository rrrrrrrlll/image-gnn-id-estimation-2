#!/bin/bash
#SBATCH --job-name=mlp_baseline_mnist
...
# Non-graph MLP baseline for MNIST (Gap 2). Trains an MLP directly on
# the Step 5 VAE embeddings -- no KNN graph involved -- and appends one
# row to results/mlp_baseline/mnist.csv. Only needs config/mlp/mnist.yaml,
...
echo "Starting MLP baseline training (MNIST)..."
python src/scripts/train_mlp_baseline.py \
    -d config/mlp/mnist.yaml \
