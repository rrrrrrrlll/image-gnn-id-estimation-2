#!/bin/bash
#SBATCH --job-name=gap_curve_fmnist
...
# Generalization-gap-vs-graph-size curve for FMNIST (Gap 1).
...
# one row per (size, seed) to results/gap_curve/fmnist.csv.
# Requires config/gnn/fmnist.yaml to point at the FMNIST KNN graph
# produced by src/scripts/gen_knn_graphs.py (README Step 9) -- same
# prerequisite as train_gnn_fmnist.sh.
echo "Starting Gap-1 curve training (FMNIST)..."
