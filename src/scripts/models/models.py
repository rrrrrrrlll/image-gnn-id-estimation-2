...
class GNNProjModel(nn.Module):
    """
    Graph Neural Network (GNN) with a fixed-width linear projection layer
    applied to the raw node features before the first GCN layer.

    Identical to GNNModel except the first entry in self.layers is an
    nn.Linear(in_features, proj_size) projection (GNNModel has this same
    line, commented out) and the first GNNBasicBlock consumes proj_size
    instead of in_features. proj_size is independent of hidden_size so a
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.layers = nn.ModuleList(
            [
                nn.Linear(in_features, proj_size),
                GNNBasicBlock(
                    proj_size, 
                    hidden_size, 
                    gnn_conv, 
...
class MLPModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # Activate every layer except the last: the last layer produces
        # class logits for torch_ce_loss (plain cross-entropy expects
        # raw, unnormalized logits), and running those through ReLU
        # would zero out every negative logit before the loss saw it.
