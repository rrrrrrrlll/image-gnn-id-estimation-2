...
class OCTMNISTDataset(OCTMNIST):
    def __init__(self, root="data/", train=True, download=True, **kwargs):
        split = "train" if train else "test"
...
class FlatEmbeddingDataset(Dataset):
    """
    Plain (non-graph) per-sample dataset over a saved embedding file in
    the format Step 5 (gen_graph_embedding.py) writes:

        [latent features... | class label]

    Each item is returned as a single-node torch_geometric.data.Data
    object (x, y, mask) so it plugs into the same forward(batch) ->
    batch.x / batch.mask convention GNNModel and MLPModel already use,
    and collates into an ordinary batch through PyG's DataLoader with
    no graph structure at all. mask is always all-True: unlike
    GraphEmbeddedDataset (which mixes train and test nodes into one
    graph and uses mask to pick out the ones that count), every row
    here already belongs to just one split, so there is nothing to
    mask out.
    """
    ...
    def __init__(self, base_dir, **kwargs):
        ...
        raw = smart_load(base_dir)

        self.x = torch.tensor(raw[:, :-1], dtype=torch.float32)
        self.targets = torch.tensor(raw[:, -1], dtype=torch.long)
    ...
    def __getitem__(self, index):
        data = Data()
        data.x = self.x[index][None, :]
        data.y = self.targets[index][None]
        data.mask = torch.ones(1, dtype=torch.bool)
