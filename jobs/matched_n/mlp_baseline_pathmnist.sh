#!/bin/bash
# Matched-training-size MLP baseline for pathmnist (Gap 2).
#
...
# directly (bash mlp_baseline_pathmnist.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/mlp_pathmnist.log") 2>&1

echo "Starting matched-n MLP baseline training (pathmnist)..."
