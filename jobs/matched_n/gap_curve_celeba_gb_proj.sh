#!/bin/bash
# Matched-training-size gap curve for celeba_gb (Gap 1) using GNNProjModel --
...
# results?). Otherwise identical to gap_curve_celeba_gb.sh.
#
...
mkdir -p log
exec > >(tee -a "log/gap_celeba_gb_proj.log") 2>&1

echo "Starting matched-n gap-curve training (GNNProjModel, celeba_gb)..."
