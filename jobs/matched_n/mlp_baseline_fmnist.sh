#!/bin/bash
# Matched-training-size MLP baseline for fmnist (Gap 2).
#
...
# directly (bash mlp_baseline_fmnist.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/mlp_fmnist.log") 2>&1

echo "Starting matched-n MLP baseline training (fmnist)..."
