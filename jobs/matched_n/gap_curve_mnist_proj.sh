#!/bin/bash
# Matched-training-size gap curve for mnist (Gap 1) using GNNProjModel --
...
# results?). Otherwise identical to gap_curve_mnist.sh.
#
...
mkdir -p log
exec > >(tee -a "log/gap_mnist_proj.log") 2>&1

echo "Starting matched-n gap-curve training (GNNProjModel, mnist)..."
