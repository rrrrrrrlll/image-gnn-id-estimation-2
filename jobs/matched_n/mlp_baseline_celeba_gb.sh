#!/bin/bash
# Matched-training-size MLP baseline for celeba_gb (Gap 2).
#
...
# directly (bash mlp_baseline_celeba_gb.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/mlp_celeba_gb.log") 2>&1

echo "Starting matched-n MLP baseline training (celeba_gb)..."
