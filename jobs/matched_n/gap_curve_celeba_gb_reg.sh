#!/bin/bash
# Matched-training-size gap curve for celeba_gb (Gap 1), with ID regularization --
# CE + lambda_id * id_penalty(embeddings, "mle") via
# config/gnn_training_config_reg.yaml (see training/id_regularizers.py).
# Otherwise identical to gap_curve_celeba_gb.sh; results land in a separate
# directory so they never mix with the unregularized runs.
#
...
# directly (bash gap_curve_celeba_gb_reg.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/gap_celeba_gb_reg.log") 2>&1

echo "Starting matched-n gap-curve training (ID-regularized, celeba_gb)..."
python src/scripts/train_gap_curve.py \
    ...
    --reference-n 28709 \
    --results-dir results/gap_curve/gap_curve_reg_matched_n
