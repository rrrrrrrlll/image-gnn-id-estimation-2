#!/bin/bash
# Matched-training-size MLP baseline for mnist (Gap 2).
#
...
# directly (bash mlp_baseline_mnist.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/mlp_mnist.log") 2>&1

echo "Starting matched-n MLP baseline training (mnist)..."
