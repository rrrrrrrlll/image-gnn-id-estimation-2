#!/bin/bash
# Matched-training-size MLP baseline for fer2013 (Gap 2).
#
...
# directly (bash mlp_baseline_fer2013.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/mlp_fer2013.log") 2>&1

echo "Starting matched-n MLP baseline training (fer2013)..."
