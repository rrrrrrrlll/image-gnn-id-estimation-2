#!/bin/bash
# Matched-training-size gap curve for fmnist (Gap 1) using GNNProjModel --
...
# results?). Otherwise identical to gap_curve_fmnist.sh.
#
...
# directly (bash gap_curve_fmnist_proj.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/gap_fmnist_proj.log") 2>&1

echo "Starting matched-n gap-curve training (GNNProjModel, fmnist)..."
