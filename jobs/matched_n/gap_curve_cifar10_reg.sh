#!/bin/bash
# Matched-training-size gap curve for cifar10 (Gap 1), with ID regularization --
# CE + lambda_id * id_penalty(embeddings, "mle") via
# config/gnn_training_config_reg.yaml (see training/id_regularizers.py).
# Otherwise identical to gap_curve_cifar10.sh; results land in a separate
...
# directly (bash gap_curve_cifar10_reg.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/gap_cifar10_reg.log") 2>&1

echo "Starting matched-n gap-curve training (ID-regularized, cifar10)..."
