#!/bin/bash
# Matched-training-size gap curve for pathmnist (Gap 1) using GNNProjModel --
...
# results?). Otherwise identical to gap_curve_pathmnist.sh.
#
...
mkdir -p log
exec > >(tee -a "log/gap_pathmnist_proj.log") 2>&1

echo "Starting matched-n gap-curve training (GNNProjModel, pathmnist)..."
python src/scripts/train_gap_curve.py \
    ...
    --reference-n 28709 \
    --results-dir results/gap_curve/gap_curve_matched_n_proj
