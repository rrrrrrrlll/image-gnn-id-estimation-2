#!/bin/bash
# Matched-training-size gap curve for celeba_sb (Gap 1) using GNNProjModel --
...
# results?). Otherwise identical to gap_curve_celeba_sb.sh.
#
...
mkdir -p log
exec > >(tee -a "log/gap_celeba_sb_proj.log") 2>&1

echo "Starting matched-n gap-curve training (GNNProjModel, celeba_sb)..."
