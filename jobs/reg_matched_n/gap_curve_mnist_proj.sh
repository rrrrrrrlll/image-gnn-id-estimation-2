#!/bin/bash
# Matched-training-size gap curve for mnist (Gap 1) using GNNProjModel, with
...
# Otherwise identical to jobs/matched_n/gap_curve_mnist_proj.sh. Results land
...
# jobs/matched_n/gap_curve_mnist_proj_reg.sh run (different fraction/restart
...
# directly (bash gap_curve_mnist_proj.sh) / in the background (nohup ... &).
#
...
mkdir -p log
exec > >(tee -a "log/gap_mnist_proj_reg.log") 2>&1

echo "Starting matched-n gap-curve training (ID-regularized GNNProjModel, mnist)..."
