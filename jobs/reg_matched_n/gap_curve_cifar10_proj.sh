#!/bin/bash
# Matched-training-size gap curve for cifar10 (Gap 1) using GNNProjModel, with
...
# Otherwise identical to jobs/matched_n/gap_curve_cifar10_proj.sh. Results land
...
# jobs/matched_n/gap_curve_cifar10_proj_reg.sh run (different fraction/restart
...
mkdir -p log
exec > >(tee -a "log/gap_cifar10_proj_reg.log") 2>&1

echo "Starting matched-n gap-curve training (ID-regularized GNNProjModel, cifar10)..."
